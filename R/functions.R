...
get_land <- function(GCAM_version = "v7.1") {
  ...
  cereal_scalar <- get(paste('cereal_scaler',GCAM_version,sep='_'))
  land_filter <- get(paste('land_use_map',GCAM_version,sep='_'), envir = asNamespace("gcamreport"))
  ...
  land_tmp <-
    ...
    dplyr::filter(crop %in% land_filter$crop) %>%
    dplyr::mutate(water = dplyr::if_else(water == crop, NA_character_,water)) %>%
